...
export default function BookingPreview({ selectedDate, selectedEndDate, selectedRoom, selectedTimeSlot, participants, roomType, hasValidationErrors, enableDateRange = false }) {
  ...
  // Formaterer dato-range
  const formatDateRange = () => {
    if (!selectedDate) return 'Ikke valgt'
    if (!enableDateRange || !selectedEndDate) {
      return formatDateForPreview(selectedDate)
    }
    return `${formatDateForPreview(selectedDate)} - ${formatDateForPreview(selectedEndDate)}`
  }
    ...
    <div style={{ 
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: '0.75rem' }}>
        <div style={{ display: 'flex', flexDirection: 'column', gap: '0.25rem', fontSize: '0.875rem' }}>
          <span style={{ fontWeight: 500, color: '#374151' }}>Dato{enableDateRange && selectedEndDate ? 'r' : ''}:</span>
          <span style={{ 
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '0.25rem', fontSize: '0.875rem' }}>
          <span style={{ fontWeight: 500, color: '#374151' }}>Lokale type:</span>
          <span style={{ 
            backgroundColor: '#ffffff', 
            border: '1px solid #000000', 
            padding: '0.5rem', 
