...
export function useBookingController(options = {}) {
  ...
  // timeSlots/roomType kan overskrives via hookOptions fra siden (fx mødelokale vs undervisningslokale)
  // enableDateRange afgør om man kan vælge en dato-periode (kun undervisningslokaler)

  // UI-state: det der driver komponenterne på bookingsiden
  ...
  // Rolle hentes fra sessionStorage (sat ved login). Bruges til fx admin-adfærd i UI
  const [userRole, setUserRole] = useState(() => {
    if (typeof window !== 'undefined') {
      return sessionStorage.getItem('userRole') || 'studerende';
    }
    return 'studerende';
  })
  ...
  useEffect(() => {
    const fetchBookings = async () => {
      ...
      try {
        ...
        // Vi henter et bredere interval så bookinger der går over midnat også fanges
        ...
        const { data, error } = await supabase
          .from('session-table')
          .select('starts_at, ends_at') // Vi henter kun tiderne (nok til at markere slots som optaget)
        ...
        if (data && data.length > 0) {
          ...
          // Set bruges for at undgå dubletter, hvis flere bookinger overlapper samme slot
          ...
          data.forEach(booking => {
            ...
            if (!bookingStart) {
              // Bagudkompatibilitet: ældre rækker kan mangle starts_at, så vi matcher kun på sluttid
            ...
            datesToCheck.forEach(checkDate => {
              timeSlots.forEach(slot => {
                ...
                // Hvis tidsrummet går over midnat, flyt slutdatoen til næste dag
  ...
  const calculateEndTime = (date, timeSlot) => {
    ...
    // Tjek om sluttidspunktet ligger "før" starttidspunktet -> så krydser slot'et midnat
  ...
  const handleConfirmBooking = async () => {
    try {
      ...
      // booked_by gemmer email på den bruger der opretter bookingen (hentes fra sessionStorage)
      ...
      // (én database-række pr. dato i perioden)
      const bookingDataArray = datesToBook.map(date => {
        ...
        return {
          ...
          booked_by: bookedBy,
          participants: JSON.stringify(participants), // Gemmer deltagerliste som JSON-string
          room_type: roomType
        }
      })
  ...
  const handleDeleteBooking = async () => {
    ...
    try {
      ...
      // Sletter ud fra room_id + ends_at (antager at kombinationen peger på den rigtige booking)
